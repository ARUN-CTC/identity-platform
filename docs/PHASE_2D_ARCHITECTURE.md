...
**Status: this document itself remains architecture and ADR only, as originally written and gate-reviewed (§9/§10 below).** Eleven implementation sub-phases have since been built: **2D.1 (cryptographic foundation and legacy/external token trust boundary, `docs/PHASE_2D1.md`)**, **2D.2 (Application/OAuth Client foundation — registration, lifecycle, grant/scope/audience/redirect-URI/origin policy enforcement, `docs/PHASE_2D2.md`)**, **2D.3 (ServiceAccount & Tenant Grant foundation — machine identity, per-tenant authorization, `docs/PHASE_2D3.md`)**, **2D.4 (OAuth 2.0 Client Credentials flow — `POST /oauth/token`, the first endpoint that actually issues an external token, `docs/PHASE_2D4.md`)**, **2D.5 (Resource Server JWT Validation & Authorization Context — the reusable consuming-side validator/guard, `docs/PHASE_2D5.md`, `docs/RESOURCE_SERVER_ARCHITECTURE.md`)**, **2D.6 (Product Resource Authorization Contract & Integration Boundary — the reusable, product-neutral policy/scope/entitlement contract layered on 2D.5's principal, `docs/PHASE_2D6.md`, `docs/RESOURCE_AUTHORIZATION_CONTRACT.md`)**, **2D.7 (OAuth Authorization Code + PKCE — `GET /oauth/authorize` + `POST /oauth/token` with `grant_type=authorization_code`, the human-user counterpart to 2D.4's machine flow, `docs/PHASE_2D7.md`, `docs/OAUTH_AUTHORIZATION_CODE_PKCE.md`)**, **2D.8 (OIDC Provider — `openid` scope, mandatory nonce, ID Token issuance, `/userinfo`, discovery, layered additively on 2D.7's flow, `docs/PHASE_2D8.md`, `docs/OIDC_PROVIDER.md`)**, and **2D.9 (OAuth/OIDC Operational Hardening & Consent Boundary — rate limiting, resource-consumption limits, request correlation, security observability, a metrics abstraction, authorization-code lifecycle cleanup, an inert consent-boundary seam, JWKS operational review, and production configuration validation, layered on 2D.1-2D.8 without any protocol/architecture change, `docs/PHASE_2D9.md`, `docs/OAUTH_OPERATIONAL_HARDENING.md`)**, and **2D.10 (External Product Integration Contract — the stable, product-facing naming of the existing principal/pipeline/error/entitlement shapes (`src/contracts/`), a documented wire-level contract, and a defined-but-unbuilt SDK boundary, entirely additive to 2D.1-2D.9, `docs/PHASE_2D10.md`, `docs/PRODUCT_INTEGRATION_CONTRACT.md`, `docs/IDENTITY_EXTERNAL_API_CONTRACT.md`, `docs/SDK_BOUNDARY.md`)**, and **2D.11 (Production Readiness, Resilience & Security Validation — a validation-first phase: extended production-configuration fail-closed checks (`JWT_ACCESS_SECRET`/`DATABASE_URL`/`CORS_ALLOWED_ORIGINS`, plus upper-bound "unsafe TTL/clock-skew" checks), a genuine liveness/readiness health-endpoint split, three real check-then-insert race conditions found and fixed (`Tenant`/`Product`/`User` creation), a live-database RLS/role verification, a validated backup/restore procedure and script, and a documented dependency-failure/load-test/deployment baseline — zero protocol change, `docs/PHASE_2D11.md`, `docs/PRODUCTION_READINESS.md`, `docs/RESILIENCE_AND_FAILURE_MODEL.md`, `docs/SECURITY_OPERATIONS_RUNBOOK.md`)** — see the updated Implementation Readiness Matrix rows in §10. Every other row/sub-phase remains design-only, unimplemented, exactly as this document originally specified.
...
2D.6  OIDC ID Token issuance (openid scope, nonce, auth_time) — ADR-014 — ✅ COMPLETE
      (built as Phase 2D.8, docs/PHASE_2D8.md, docs/OIDC_PROVIDER.md — `openid`
      scope trigger, mandatory nonce bound to the Phase 2D.7 authorization
      code, ID Token issuance via a new narrowly-scoped `IdTokenService`
      reusing Phase 2D.1's signing infrastructure, `/userinfo`. `auth_time`
      NOT emitted — this platform does not currently capture the underlying
      session's own original authentication timestamp on the authorization
      code, and the phase's own brief explicitly permits omitting rather
      than fabricating it; `acr`/`amr` also NOT emitted, same reasoning)
2D.7  OIDC discovery document (/.well-known/openid-configuration) — JWKS itself already shipped in 2D.1 — ✅ COMPLETE
      (built as Phase 2D.8, alongside 2D.6 above — the two were implemented
      together since discovery is only meaningful once ID Token issuance
      exists to advertise)
...
**Note on implementation order vs. this list's numbering**: the actual build sequence named its third through eighth implementation sub-phases **"2D.3"** (`docs/PHASE_2D3.md`), **"2D.4"** (`docs/PHASE_2D4.md`), **"2D.5"** (`docs/PHASE_2D5.md`), **"2D.6"** (`docs/PHASE_2D6.md`), **"2D.7"** (`docs/PHASE_2D7.md`), and **"2D.8"** (`docs/PHASE_2D8.md`) rather than waiting for the slots numbered 2D.3-2D.5/2D.8/2D.9/2D.10 above — together the six actually-built sub-phases cover the full `ServiceAccount` identity model + token-issuance flow (2D.9, in full), the full `ServiceAccountTenantGrant` admin surface (2D.10, in full: create/list/get/patch/reactivate, tenant-first nested under `/platform/tenants/:tenantId/service-account-grants`), the resource-server validation pipeline's steps 1-9 (2D.8's ORIGINAL slot, in full — steps 1-7 the validator itself, steps 8-9 the authorization contract), this list's own "2D.3" (`/authorize`) + "2D.4" (Authorization Code + PKCE exchange), built together as Phase 2D.7, and this list's own "2D.6" (OIDC ID Token issuance) + "2D.7" (OIDC discovery), built together as Phase 2D.8. This list's own numbering is left as originally written (the *order*, not a committed calendar, per the line below) rather than renumbered after the fact; `docs/PHASE_2D3.md`/`docs/PHASE_2D4.md`/`docs/PHASE_2D5.md`/`docs/PHASE_2D6.md`/`docs/PHASE_2D7.md`/`docs/PHASE_2D8.md` are the authoritative record of what was actually built and when.
...
| OIDC | ADR-014 — additive ID Token, distinct from access token | Depends on OAuth AS (above) | ✅ **IMPLEMENTED** (Phase 2D.8 — `openid` scope trigger, mandatory nonce, `IdTokenService`, `/userinfo`, discovery, `docs/PHASE_2D8.md`, `docs/OIDC_PROVIDER.md`; `aud`=OIDC client_id vs. Access Token's `aud`=resource API kept structurally distinct throughout) |
...
| Resource servers | ADR-020 — 11-step pipeline, fully ordered, fully specified | Depends on JWKS + audience registration | ✅ **steps 1-7 IMPLEMENTED** (Phase 2D.5 — `ExternalAccessTokenValidator`/`JwksClientService`/`ExternalBearerAuthGuard`, `docs/PHASE_2D5.md`; Phase 2D.7 extended the validator to distinguish `USER` from `SERVICE_ACCOUNT` principals via an explicit `principal_type` claim, with zero behavior change for any pre-existing machine token, `docs/PHASE_2D7.md` §15; Phase 2D.8 further extended it to reject an OIDC ID Token outright via an explicit `token_use` claim, again with zero behavior change for any pre-existing token, `docs/PHASE_2D8.md`); ✅ **steps 8-9's CONTRACT (not any product's implementation) now exists** (Phase 2D.6 — `ResourceAuthorizationPolicy`/`ResourceAuthorizationGuard`, `docs/PHASE_2D6.md`, unmodified by and fully compatible with both the USER principal type and the token_use extension); step 11 (tenant-scoped data access, each product's own RLS/isolation) remains, correctly, entirely product-owned and out of scope for Identity Platform to provide |
| Refresh tokens | Existing Phase 1–2C mechanism, reused as-is | None | ✅ |
| Operational hardening | Rate limiting, resource limits, observability, config validation — not named by any prior ADR, added as production-readiness work | None — additive to 2D.1-2D.8 | ✅ **IMPLEMENTED** (Phase 2D.9 — `docs/PHASE_2D9.md`, `docs/OAUTH_OPERATIONAL_HARDENING.md`) |
| External product integration contract | Stable product-facing naming for the existing pipeline/principal/error shapes; SDK boundary definition | None — additive, zero protocol change | ✅ **IMPLEMENTED** (Phase 2D.10 — `docs/PHASE_2D10.md`, `docs/PRODUCT_INTEGRATION_CONTRACT.md`, `docs/IDENTITY_EXTERNAL_API_CONTRACT.md`, `docs/SDK_BOUNDARY.md`, `src/contracts/`) |
